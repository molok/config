...
Irssi::theme_register([
  ...
  'join_realname', '{channick_hilight $0} ({hilight $1} - {chanhost_hilight $2 $3}) has joined {channel $4}',
...
sub event_whois {
  ...
  if (!($rec->{nicks}->{$nick}->{state} & 2)) {
    ...
    foreach my $channel (@{$rec->{nicks}->{$nick}->{chans_join}}) {
      ...
      my $nickrec = $chanrec->nick_find($nick);
      if ($chanrec && $nickrec) {
        $chanrec->printformat(MSGLEVEL_JOINS, 'join_realname', $nick, $realname, $host, $nickrec->{host}, $channel);
