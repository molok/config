...
%IRSSI = (
	...
	description      => q(Adds a permanent advanced window list on the right or ).
	                    q(in a statusbar.),
	description2     => q(Based on chanact.pl and nicklist.pl with various other ).
	                    q(ideas from random scripts.),
...
#     /set awl_display_nokey_active <string> XXX NOT YET IMPLEMENTED
#     /set awl_display_key_active <string> XXX NOT YET IMPLEMENTED
...
sub remake () {
	...
	foreach my $win (@wins) {
		...
		$display = (defined $keymap{$number} and $keymap{$number} ne '')
				?
			(
				Irssi::settings_get_str(set 'display_key')
					||
				Irssi::settings_get_str(set 'display_nokey')
			)
				:
			Irssi::settings_get_str(set 'display_nokey')
		;
